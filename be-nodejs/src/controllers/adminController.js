...
export const getAllAlerts = async (req, res) => {
  try {
    const { page = 0, size = 50, userId, symbol, isActive, search } = req.query;
    const { AlertModel } = await import('../models/alertModel.js');
    ...
    if (search) {
      query += ` AND (a.symbol ILIKE $${paramIndex} OR a.notes ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }
    ...
    res.json(createResponse(true, 'All alerts retrieved', {
